...
HikRobot::HikRobot(double exposure_ms, double gain, const std::string &vid_pid,
                   unsigned int adcbitdepth, double framerate,
                   bool trigger_mode)
    : exposure_us_(exposure_ms * 1e3), gain_(gain), adcbitdepth_(adcbitdepth),
      framerate_(framerate), queue_(1), daemon_quit_(false), vid_(-1), pid_(-1),
  ...
  daemon_thread_ = std::thread{[this] {
    ...
    while (!daemon_quit_) {
      ...
      if (retry_count_++ >= 10) {
        tools::logger()->warn(
            "HikRobot capture thread failed to capture for 10 times, giving up.");
        exit(1);
      }
      capture_stop();
      reset_usb();
...
void HikRobot::capture_start() {
  ...
  unsigned int adcbitdepth_enum = 0; // default 8bit
  if (adcbitdepth_ == 8) {
    adcbitdepth_enum = 0;
  } else if (adcbitdepth_ == 12) {
    adcbitdepth_enum = 3;
  } else {
    tools::logger()->warn("Unsupported ADC bit depth: {}, using default 8bit",
                          adcbitdepth_);
    adcbitdepth_enum = 0;
  }
  set_enum_value("ADCBitDepth", adcbitdepth_enum);
  set_enum_value("GainAuto", MV_GAIN_MODE_OFF);
  set_float_value("ExposureTime", exposure_us_);
  set_float_value("Gain", gain_);
  MV_CC_SetFrameRate(handle_, framerate_);

  if (trigger_mode_) {
    set_enum_value("LineSelector", 2);      // 选择Line2
    set_enum_value("TriggerMode", 1);       // 1 = On 触发模式
    set_enum_value("TriggerSource", 2);     // 2 = Line2
    set_enum_value("TriggerActivation", 0); // 0 = RisingEdge 上升沿
  } else {
    set_enum_value("TriggerMode", 0);
  }
