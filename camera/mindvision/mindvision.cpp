...
{
  int camera_num = 4;
  tSdkCameraDevInfo camera_info_list[4];
  ...
  for (int i = 0; i < camera_num; i++) {
    std::string camera_name = camera_info_list[i].acFriendlyName;
    tools::logger()->debug("Found camera {}: {}", i, camera_name);
  }

  std::string selected_camera_name = camera_info_list[0].acFriendlyName;
  tools::logger()->info("Opening first available camera: {}", selected_camera_name);
